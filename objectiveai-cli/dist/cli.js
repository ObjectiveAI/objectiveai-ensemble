...
import { existsSync, readFileSync, mkdirSync, writeFileSync, readdirSync, appendFileSync, unlinkSync, statSync } from 'fs';
...
function checkNoExtraProperties(schema, value, path) {
  if ("anyOf" in schema) {
    const zodSchemas = schema.anyOf.map(
      (s) => Functions.Expression.InputSchemaExt.toZodSchema(s)
    );
...
function ensureGitHubRepo(name, description, ghToken) {
  ...
  if (!upstream) {
    ...
    execSync("git push", { stdio: "pipe", env: ghEnv2(ghToken) });
  }
}
...
var claudeModelConfigs = [
  ...
  { key: "claudeInputSchemaModel", flag: "--claude-input-schema-model", label: "Claude Input Schema Model", desc: "Model for input schema generation" },
...
function validateCliInputSchema(inputSchemaStr, cliType) {
...
inventCmd.action(async (spec, opts) => {
  ...
  const inputSchemaStr = readIfFile(opts.inputSchema);
