...
export async function runNetworkTests(apiBase?: string, apiKey?: string): Promise<Result<undefined>> {
  ...
  if (!funcResult.ok) {
    return { ok: false, value: undefined, error: `Function validation failed: ${funcResult.error}` };
  }
  ...
  if (!profileResult.ok) {
    return { ok: false, value: undefined, error: `Profile validation failed: ${profileResult.error}` };
  }
  ...
  if (!inputsResult.ok) {
    return { ok: false, value: undefined, error: `Inputs validation failed: ${inputsResult.error}` };
  }
  ...
  try {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (result.error !== null) {
        return { ok: false, value: undefined, error: `Default strategy: execution failed for input [${i}]: ${JSON.stringify(result.error)}` };
      }
      if (result.tasks_errors) {
        return { ok: false, value: undefined, error: `Default strategy: task errors for input [${i}]` };
      }
    }
  } catch (e) {
    return { ok: false, value: undefined, error: `Default strategy: ${(e as Error).message}` };
  }
  ...
  if (func.type === "vector.function") {
    try {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (result.error !== null) {
          return { ok: false, value: undefined, error: `SwissSystem strategy: execution failed for input [${i}]: ${JSON.stringify(result.error)}` };
        }
        if (result.tasks_errors) {
          return { ok: false, value: undefined, error: `SwissSystem strategy: task errors for input [${i}]` };
        }
      }
    } catch (e) {
      return { ok: false, value: undefined, error: `SwissSystem strategy: ${(e as Error).message}` };
    }
  }
