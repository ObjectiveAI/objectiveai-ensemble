...
import { defaultVectorCompletionTaskProfile } from "../../build";
...
export function readProfile(): Result<unknown> {
  ...
  try {
    return { ok: true, value: JSON.parse(readFileSync("profile.json", "utf-8")), error: undefined };
  } catch {
    return { ok: false, value: undefined, error: "profile.json is not valid JSON" };
  }
}
...
export function validateProfile(value: unknown): Result<Functions.RemoteProfile> {
...
export function checkProfile(): Result<undefined> {
  ...
  if (!result.ok) {
    return { ok: false, value: undefined, error: `Profile is invalid: ${result.error}` };
  }
...
export function buildProfile(): Result<undefined> {
  const raw = readTasks();
  if (!raw.ok) {
    return { ok: false, value: undefined, error: `Unable to build profile: ${raw.error}` };
  }

  const tasksResult = validateTasks({ tasks: raw.value });
  if (!tasksResult.ok) {
    return { ok: false, value: undefined, error: `Unable to build profile: tasks are invalid: ${tasksResult.error}` };
  }
